...
MMD 的「表示枠」控制整模型显示 / 非表示。本标准路径：`SkeletalModel.Visible` 布尔量，
...
// 早于首键 ⇒ true（未声明「非表示」的动效应照常渲染）
// 晚于末键 ⇒ 保持末键状态
// 空轨道   ⇒ 恒 true
```

单动效走 `MmdAnimation.SampleVisible`；多动效由 `MmdAnimationLayer.IsVisibleAt(localFrame)` 取每层本地帧的
布尔，再由混合器做 **AND**（见下）。
...
- 可见性是布尔量，**不进数值混合**：即便某层 `Weight = 0.5`（数值上只贡献一半姿态），
  它的「非表示」依旧一票否决——规避 babylon-mmd 把可见性当 0.5 半透明处理的 bug。
...
> 屏幕上仍会出现一个已经不可见模型的影子。因此 `GlesShadowRenderer` 在跳过前显式 `glClear(DepthBufferBit)`
> （空处 z=1，采样即「不在影里」）。隐藏模型不能只早退——必须清深度。
