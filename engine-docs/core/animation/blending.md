...
| `IsVisibleAt(localFrame)`   | 方法             | 该层本地帧处的显示帧（显示 / 不显示）                    |

> `IsActive` 判定**与淡入淡出包络无关**：淡入首帧的包络为 0，但该层仍算活跃。
> 原因——可见性是布尔量，不参与数值混合，一票否决不因 fade 而失效。
...
`Evaluate` 之后仍需调用 `MmdMorphEvaluator.Evaluate(model)`，
骨 morph / 材质 morph 才会落地（见 [lifecycle.md](lifecycle.md)）。

### 混合规则

- **旋转**：半球对齐 nlerp。累加前若与已累计方向 dot < 0 取负（走最短弧），
  保证结果与累加顺序无关。
- **残差**：按「覆盖该项的权重和」混回绑定姿势，而不是全局权重。
- **可见性**：布尔 AND，**不做数值加权**。
  显示帧里的 IK 开关只解析保留（`MmdAnimation.IkStates`），未参与求值。

### 权重归一：逐项而非全局

归一化只作用在**同一骨 / morph** 上：仅当该项的覆盖权重和 `W(item) > 1` 时才除以 `W(item)`；
`W ≤ 1` 走残差混回绑定。要点：

- **槽位不重叠时保持满强度**——MMD 的「动作 + 表情槽」各层轨道通常互不重叠，
  若用全局 `Σw` 归一，四层动画会被各压到 1/4 强度。
- **层间竞争**（多层驱动同一项）时，逐项归一与全局归一结果等价。
- **单层满权直通**：`boneN == 1 && w == 1` 时跳过归一化，与单动效
  `MmdAnimation.Sample` **位级一致**——这条等价性必须保持，不要"优化"掉。

累加器的中性元是**零向量**（不是 Identity）：用 Identity 初始化会把贡献叠成 2 倍；
