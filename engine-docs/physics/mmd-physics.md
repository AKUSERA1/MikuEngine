...
reze `RezePhysics` 逐行移植，按渲染帧序拆成三个相位，由 `Update` 一次串起：
...
- 预计算：每个动态体在关节图上的 kinematic 根（teleport carry 用）、
  mode-2 对齐钉扎集（`alignPinned`：直接挂在跟随骨骼体上的 PMX mode-2 体，如 `胸_回転`）。
...
| `boneWorldMatrices` | 骨骼世界矩阵（列主序 `float[16×骨数]`，FK/IK/付与结果）；会被**就地写回** |
...
- `SetKinematicTargets`：从骨骼世界矩阵导出 kinematic 目标位姿与速度。速度取自
  **帧间目标轨迹**（而非子步推进量）——避免 60Hz 以上渲染率下累加器相位纹波激励布料链。
...
时间轴拖动 / 乱序模拟后调用，做三件事（照搬 reze，不"直觉"改写）：
...
- **SetJiggleDamping**：把指定骨骼所载刚体的阻尼乘以 `scale`（1 = 还原作者值，0.5 = 减半，
  ...
  Stage 1 不接出 UI，API 预留。
- **GetPhysicsDrivenBones**：列出"本模拟每步覆写其骨骼世界矩阵"的骨
  （Dynamic 体绑定的真实骨）。供 [付与系统](../core/animation/append-transform.md) 的
  `SetPhysicsDrivenBones` 消费——付与亲被模拟时，付与继承必须消费模拟结果而非帧首动画姿态。
